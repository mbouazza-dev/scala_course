...
trait EncoderInstances:
  ...
  given stringEncoder: Encoder[String] =
    Encoder.fromFunction(n => Json.Str(n)) // TODO Implement the `Encoder[String]` given instance
...
trait DecoderInstances:
  ...
  given booleanDecoder: Decoder[Boolean] =
    Decoder.fromPartialFunction { case Json.Bool(b) => b.booleanValue() }
  ...
  given Decoder[Person] =
    Decoder.fromPartialFunction {
      case Json.Obj(fields) =>
        ...
        nameDecoder.zip(ageDecoder).transform {
          case (name, age) => Person(name, age) // Crée une instance de Person
        }.decode(Json.Obj(fields)).get
    }
...
trait ContactsCodecs:
  ...
  given Encoder[Contacts] =
    ObjectEncoder.fromFunction { contacts =>
      Json.Obj(Map("people" -> Json.Arr(contacts.people.map(p => Person.given_Encoder_Person.encode(p)))))
    }
  ...
  Decoder.fromFunction {
    case Json.Obj(fields) =>
      for {
        people <- Decoder.field[List[Person]]("people").decode(Json.Obj(fields))
...
@main def run(): Unit =
  ...
  println(maybeJsonObj.flatMap(_.decodeAs[String]))
