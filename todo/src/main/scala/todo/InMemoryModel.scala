...
object InMemoryModel extends Model:
  ...
  def create(task: Task): Id =
    ...
    idStore.addOne(id,Task(task.state, task.description, task.notes, task.tags))
    id
  ...
  def complete(id: Id): Option[Task] =
    idStore.updateWith(id)(opt => Option(opt.get.complete))
  ...
  def tags: Tags =
    Tags(idStore.flatMap(x => x._2._4).toList.distinct)

  def tasks(tag: Tag): Tasks =
    Tasks(idStore.filter(x => x._2.tags.contains(tag)))
